...
reference_redispatch <- c(-120, 460) / 1000
bat_eff <- .95
wtp_user <- 3000
...
<!-- title for toc -->
## - Redispatch from Active End-users {.tabset .tabset-fade .tabset-pills}
...
cols_extend <- scales::col_numeric("YlOrRd", domain = NULL)(w_extend)
start_ID <- round((min(range(redispatch_up_Mat,na.rm = T)) - w_min)^(1 / 1) / (w_max - w_min) * length(w_extend))
end_ID <- round((max(range(redispatch_up_Mat,na.rm = T)) - w_min)^(1 / 1) / (w_max - w_min) * length(w_extend))
