...
reference_redispatch <- c(-600, 600) / 1000
bat_eff <- .95
wtp_user <- 3000
...
<!-- title for toc -->
# Average Electricity Price Reduction Per Capita {.tabset .tabset-fade .tabset-pills}

## Reference  {.tabset .tabset-fade .tabset-pills}
...
surplus_supply_2 <- end_user_type_2$BESS_supply - (end_user_type_2$EOM_supply + end_user_type_2$redispatch_supply_up - end_user_type_2$redispatch_supply_down + end_user_type_2$balancing_supply_up - end_user_type_2$balancing_supply_down)
...
## Partially Flexible {.tabset .tabset-fade .tabset-pills}

### January
```{r price reduction jan part, echo = FALSE, message = FALSE, warning = FALSE}
...
surplus_supply_3 <- end_user_type_3$BESS_supply - (end_user_type_3$EOM_supply + end_user_type_3$redispatch_supply_up - end_user_type_3$redispatch_supply_down + end_user_type_3$balancing_supply_up - end_user_type_3$balancing_supply_down)
...
### July
```{r price reduction jul part, echo = FALSE, message = FALSE, warning = FALSE}
...
## Fully Flexible {.tabset .tabset-fade .tabset-pills}
...
<!-- title for toc -->
## - Redispatch from Fully Flexible {.tabset .tabset-fade .tabset-pills}

### January
```{r redispatch end jan, echo = FALSE, message = FALSE, warning = FALSE}
...
redispatch_up <- end_user_type_3$redispatch_demand_down + end_user_type_3$redispatch_supply_up
redispatch_up <- redispatch_up - end_user_type_3$redispatch_demand_up - end_user_type_3$redispatch_supply_down
redispatch_up <- redispatch_up / 100 / point_data$pop_density / end_user_types_active$X3[1]

redispatch_up_Mat <- as.matrix(point_matrix)
for(row_iter in 1:nrow(redispatch_up_Mat)){
  for(col_iter in 1:ncol(redispatch_up_Mat)){
    if(point_matrix[row_iter, col_iter] == -1){
      redispatch_up_Mat[row_iter, col_iter] <- NA
    }else{
      redispatch_up_Mat[row_iter, col_iter] <- redispatch_up[point_matrix[row_iter, col_iter]]
    }
  }
}
redispatch_up_Mat <- t(redispatch_up_Mat)

### Parameters for the field
w_min <- min(min(range(redispatch_up_Mat,na.rm = T)), reference_redispatch[1])
w_max <- max(max(range(redispatch_up_Mat,na.rm = T)), reference_redispatch[2])
w_max <- (w_max - w_min)^(1 / 1)
w_extend <- seq(0, w_max, length.out = 10000)
cols_extend <- scales::col_numeric("RdYlGn", domain = NULL)(w_extend)
start_ID <- round((min(range(redispatch_up_Mat,na.rm = T)) - w_min)^(1 / 1) / w_max * length(w_extend))
end_ID <- round((max(range(redispatch_up_Mat,na.rm = T)) - w_min)^(1 / 1) / w_max * length(w_extend))
end_ID <- min(end_ID, 10000)
w_vals <- w_extend[start_ID:end_ID]
w_vals_norm <- unique(scales::rescale(c(w_vals)))
cols <- cols_extend[start_ID:end_ID]
colz <- setNames(data.frame(w_vals_norm, cols), NULL)
...
NO_field_plot <- NO_power_system_plot %>%
  add_trace(x = seq(x_range_field [1], x_range_field [2], res_field),
  y = seq(y_range_field [1], y_range_field [2], res_field),
  z = redispatch_up_Mat, type = "heatmap",
  hoverinfo = 'none',
  showscale = F,
  colorscale = colz)
...
NO_field_plot <- NO_field_plot %>%
  ...
  text = paste("\nlon: ", round(point_data$lon, 3), "°",
               "\nlat: ", round(point_data$lat, 3), "°",
               "\n\ntotal upward redispatch:\n", round(redispatch_up * 1000, 3),
               " kWh",
               sep = ""),
  hoverinfo = 'text'
  )
...
NO_field_plot <- NO_field_plot %>%
  add_text(x = node_data$x[132] + 150000, y = node_data$y[116] - 25000 + 100000 + 50000,
  textfont = list(size = 18), text = "Total Net Redispatch (kWh / ppl):", textposition = "top right", hoverinfo = 'none')
palette_level <- cols_extend[c(1, seq(.25, .75, .25) * length(w_extend), length(w_extend))]
redispatch_up_level <- round(w_extend[c(1, seq(.25, .75, .25) * length(w_extend), length(w_extend))]^1 + w_min, 3)
for(level_iter in 1:length(redispatch_up_level)){
  NO_field_plot <- NO_field_plot %>%
    add_trace(x = node_data$x[132] + 50000 + 100000 * level_iter + 60000 * c(0: 1), y = rep(node_data$y[116] + 100000, 2),
    type = 'scatter', mode = 'lines', line = list(color = palette_level[level_iter], width = 10),
    hoverinfo = 'none'
  ...
  NO_field_plot <- NO_field_plot %>%
    add_text(x = node_data$x[132] + 80000 + 100000 * level_iter, y = node_data$y[116] - 25000 + 100000,
    textfont = list(size = 14), text = redispatch_up_level[level_iter] * 1000, textposition = "bottom center", hoverinfo = 'none')
}
...
### July
```{r redispatch end jul, echo = FALSE, message = FALSE, warning = FALSE}
...
<!-- title for toc -->
## - Redispatch from Fully Flexible 3x {.tabset .tabset-fade .tabset-pills}

### January
```{r redispatch end jan 3x, echo = FALSE, message = FALSE, warning = FALSE}

setwd(parent_dir)

scenario_name <- "full_3x"
time_name <- "0101-0114"
dir_name <- paste(getwd(), "/data/output/", scenario_name, "/", time_name, "/agent/", sep = "")
...
### July
```{r redispatch end jul 3x, echo = FALSE, message = FALSE, warning = FALSE}
