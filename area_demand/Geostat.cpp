...
// Function definition
Vector3d xyz_transform(Vector2d uv){
	//	x = cos(u) * sin(v)
	//	y = sin(u) * sin(v)
	//	z = cos(v)
...
Vector2d uv_transform(Vector3d xyz){
	// u = atan2(y , x) 
	// v = acos(z)
...
double geodist(Vector2d P_1, Vector2d P_2){
	
	Vector2d s_1 = P_1;
	Vector2d s_2 = P_2;
	s_1(1) = pi / 2 - P_1(1);
	s_2(1) = pi / 2 - P_2(1);
	Vector3d x_1 = xyz_transform(s_1);
	Vector3d x_2 = xyz_transform(s_2);
