...
namespace local{
	...
	void power_supplier_settlement_print(power_market::market_whole_inform &Power_market_inform){
		...
		Output_data = Eigen::MatrixXd::Zero(slack_HV_num, var_num);
		for(int agent_iter = 0; agent_iter < slack_HV_num; ++ agent_iter){
			Output_data.row(agent_iter) = output_row_store(var_num, Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].settlement);
			Output_data(agent_iter, 0) = Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].original_ID;
		}
		fout_name = "csv/output/agent/slack_HV_plant.csv";
		basic::write_file(Output_data, fout_name, var_names);
	}
}
