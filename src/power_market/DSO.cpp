...
void power_market::DSO_Markets_Set(markets_inform &DSO_Markets, power_network::network_inform &Power_network_inform, int Time){
	...
	for(int DSO_iter = 0; DSO_iter < DSO_Markets.size(); ++ DSO_iter){
		...
		double z_base = pow(Power_network_inform.tech_parameters.voltage_cutoff_distr, 2.) / Power_network_inform.tech_parameters.s_base / 3.;
		double partition_func = 0.;
		DSO_Markets[DSO_iter].network.num_vertice = DSO_Markets[DSO_iter].num_zone;
		Eigen::MatrixXd admittance = Eigen::MatrixXd::Zero(DSO_Markets[DSO_iter].network.num_vertice, DSO_Markets[DSO_iter].network.num_vertice);
		...
		for(int row_iter = 0; row_iter < DSO_Markets[DSO_iter].network.num_vertice - 1; ++ row_iter){
			for(int col_iter = row_iter + 1; col_iter < DSO_Markets[DSO_iter].network.num_vertice; ++ col_iter){
				...
				admittance(row_iter, col_iter) = pow(distance(row_iter, col_iter) * 1E-3, 1. + Power_network_inform.tech_parameters.fraction_dim_distr);
				partition_func += admittance(row_iter, col_iter);
			}
		}
		admittance *= Power_network_inform.tech_parameters.line_density_distr * Power_network_inform.DSO_cluster[DSO_iter].points_ID.size() * z_base;
		admittance /= partition_func * Power_network_inform.tech_parameters.z_distr_series.imag();
		std::cout << "-------------------------------------------------------------------------------------------------------------------------------------------------\n";
		std::cout << DSO_iter << "\n";
		std::cout << "-------------------------------------------------------------------------------------------------------------------------------------------------\n";
		std::cout << admittance.topLeftCorner(10, 10) << "\n\n";
		std::cout << distance.topLeftCorner(10, 10) << "\n\n";
