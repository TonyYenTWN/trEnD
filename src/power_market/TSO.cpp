...
void power_market::TSO_Market_Set(market_inform &TSO_Market, power_network::network_inform &Power_network_inform, int Time){
	...
	// Set zone names
	TSO_Market.zone_names.reserve(TSO_Market.num_zone);
	for(int zone_iter = 0; zone_iter < TSO_Market.num_zone; ++ zone_iter){
		std::string zone_name = "Node_" + std::to_string(zone_iter);
		TSO_Market.zone_names.push_back(zone_name);
	}
	...
	TSO_Market.network.voltage_constraint.col(0) *= -Power_network_inform.tech_parameters.theta_trans_limit;
	TSO_Market.network.voltage_constraint.col(1) *= Power_network_inform.tech_parameters.theta_trans_limit;
	...
	TSO_Market.imbalance.supply_up = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
	TSO_Market.imbalance.supply_down = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
	TSO_Market.imbalance.demand_up = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
	TSO_Market.imbalance.demand_down = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
	TSO_Market.balancing.cost = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
	TSO_Market.balancing.utility = Eigen::MatrixXd::Zero(Time, TSO_Market.num_zone);
...
//	}
	...
	for(int agent_iter = 0; agent_iter < slack_HV_num; ++ agent_iter){
		...
		Power_market_inform.TSO_Market.submitted_supply.col(node_ID) += Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].bids.redispatch_supply;
		Power_market_inform.TSO_Market.submitted_demand.col(node_ID) += Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].bids.redispatch_demand;
	}
...
void power_market::Balancing_bid_calculation(int tick, market_whole_inform &Power_market_inform, power_network::network_inform &Power_network_inform){
	...
	for(int agent_iter = 0; agent_iter < slack_HV_num; ++ agent_iter){
		...
		Power_market_inform.TSO_Market.submitted_supply.col(node_ID) += Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].bids.balancing_supply;
		Power_market_inform.TSO_Market.submitted_demand.col(node_ID) += Power_market_inform.agent_profiles.power_supplier.slack.HV_plant[agent_iter].bids.balancing_demand;
	}
}
