...
void power_network::HELM_Node_Update(int tick, network_inform &Power_network_inform, power_market::market_whole_inform &Power_market_inform){
	...
	for(int agent_iter = 0; agent_iter < slack_LV_num; ++ agent_iter){
		...
		double real_power = Power_market_inform.agent_profiles.power_supplier.slack.LV_plant[agent_iter].results.actual_supply;
		real_power /= 1. - power_network::parameters::loss_factor();
		real_power -= Power_market_inform.agent_profiles.power_supplier.slack.LV_plant[agent_iter].results.actual_demand;
		Power_network_inform.power_flow.P_node(tick, node_ID) += real_power;
	}
}
